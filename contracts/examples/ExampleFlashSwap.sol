...
import '@etcswap/v2-core/contracts/interfaces/IETCswapV2Callee.sol';
...
contract ExampleFlashSwap is IETCswapV2Callee {
    ...
    constructor(address _factory, address _factoryV1, address router) public {
        ...
        factory = _factory;
        WETC = IWETC(IETCswapV2Router01(router).WETC());
    }
    ...
    function etcswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        address[] memory path = new address[](2);
        uint amountToken;
        uint amountETC;
        { // scope for token{0,1}, avoids stack too deep errors
        address token0 = IETCswapV2Pair(msg.sender).token0();
        address token1 = IETCswapV2Pair(msg.sender).token1();
        assert(msg.sender == ETCswapV2Library.pairFor(factory, token0, token1)); // ensure that msg.sender is actually a V2 pair
        ...
        IETCswapV1Exchange exchangeV1 = IETCswapV1Exchange(factoryV1.getExchange(address(token))); // get V1 exchange

        if (amountToken > 0) {
            (uint minETC) = abi.decode(data, (uint)); // slippage parameter for V1, passed in by caller
            token.approve(address(exchangeV1), amountToken);
            uint amountReceived = exchangeV1.tokenToEthSwapInput(amountToken, minETC, uint(-1));
            uint amountRequired = ETCswapV2Library.getAmountsIn(factory, amountToken, path)[0];
            ...
            uint amountReceived = exchangeV1.ethToTokenSwapInput{value: amountETC}(minTokens, uint(-1));
            uint amountRequired = ETCswapV2Library.getAmountsIn(factory, amountETC, path)[0];
